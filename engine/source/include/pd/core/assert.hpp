...
#define PD_ASSERT(expr) \
  ((expr) ? static_cast<void>(0) : (pd::assert_fail(#expr, "", __FILE__, __LINE__, __FUNCTION__), PD_DEBUG_BREAK()))
#define PD_ASSERT_MSG(expr, msg) \
  ((expr) ? static_cast<void>(0) : (pd::assert_fail(#expr, msg, __FILE__, __LINE__, __FUNCTION__), PD_DEBUG_BREAK()))
#else
...
namespace pd {
inline void assert_fail(const char* expr, const char* what, const char* file, int line, const char* function) {
