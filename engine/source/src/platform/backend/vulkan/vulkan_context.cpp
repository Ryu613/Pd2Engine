...
bool VulkanContext::initialize(const VulkanConfig& vulkanConfig) noexcept {
  volkInitialize();
  vkb::InstanceBuilder instanceBuilder;
  auto builder = instanceBuilder.set_app_name(vulkanConfig.appName.c_str())
                     .use_default_debug_messenger()
                     .require_api_version(1, 3, 0)
                     .enable_extensions(vulkanConfig.requiredInstanceExtensions);
  ...
  if (!createSurface(vulkanConfig.nativeWindowHandle)) {
    log::error("vulkan surface creation failed!");
    return false;
  }
