...
//  [!] Renderer: User texture binding. Use a VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE
//  'VkDescriptorSet' as texture identifier. Call ImGui_ImplVulkan_AddTexture() to
//  register one. Read the FAQ about ImTextureID/ImTextureRef +
//  https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer: Large meshes
//  support (64k+ vertices) even with 16-bit indices
//  (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Texture updates support for
//  dynamic font atlas (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Expose
//  selected render state for draw callbacks to use. Access in
//  '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'.

// The aim of imgui_impl_vulkan.h/.cpp is to be usable in your engine without any
// modification. IF YOU FEEL YOU NEED TO MAKE ANY CHANGE TO THIS CODE, please share them
// and your feedback at https://github.com/ocornut/imgui/

// You can use unmodified imgui_impl_* files in your project. See examples/ folder for
// examples of using this. Prefer including the entire imgui/ repository into your project
// (either as a copy or as a submodule), and only build the backends you need. Learn about
// Dear ImGui:
...
// Important note to the reader who wish to integrate imgui_impl_vulkan.cpp/.h in their
// own engine/app.
// - Common ImGui_ImplVulkan_XXX functions and structures are used to interface with
// imgui_impl_vulkan.cpp/.h.
//   You will use those if you want to use this rendering backend in your engine/app.
// - Helper ImGui_ImplVulkanH_XXX functions and structures are only used by this example
// (main.cpp) and by
//   the backend itself (imgui_impl_vulkan.cpp), but should PROBABLY NOT be used by your
//   own engine/app code.
// Read comments in imgui_impl_vulkan.h.

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2026-04-23: Added support for standard draw callbacks (in platform_io):
//  DrawCallback_ResetRenderState, DrawCallback_SetSamplerLinear,
//  DrawCallback_SetSamplerNearest. (#9378) 2026-04-22: *BREAKING CHANGE* redesigned to
//  use separate ImageView + Sampler instead of Combined Image Sampler. This change allows
//  us to facilitate changing samplers, in line with other backends.
//              - When registering custom textures: changed ImGui_ImplVulkan_AddTexture()
//              signature to remove Sampler.
//                - Before: ImGui_ImplVulkan_AddTexture(VkSampler, VkImageView,
//                VkImageLayout)
//                - After:  ImGui_ImplVulkan_AddTexture(VkImageView, VkImageLayout)
//                - Kept inline redirection function that ignores the sampler.
//              - When creating your own descriptor pool (instead of letting backend
//              creates its own):
//                - Before: need at least
//                IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE descriptors of type
//                VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER.
//                - After:  need at least
//                IMGUI_IMPL_VULKAN_MINIMUM_SAMPLED_IMAGE_POOL_SIZE descriptors of type
//                VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE.
//                                      + IMGUI_IMPL_VULKAN_MINIMUM_SAMPLER_POOL_SIZE
//                                      descriptors of type VK_DESCRIPTOR_TYPE_SAMPLER.
//  2026-03-11: Vulkan: Added ImGui_ImplVulkan_PipelineInfo::ExtraDynamicStates[] to allow
//  specifying extra dynamic states to add when creating the VkPipeline. (#9211)
//  2025-09-26: [Helpers] *BREAKING CHANGE*: Vulkan: Helper
//  ImGui_ImplVulkanH_DestroyWindow() does not call vkDestroySurfaceKHR(): as surface is
//  created by caller of ImGui_ImplVulkanH_CreateOrResizeWindow(), it is more consistent
//  that we don't destroy it. (#9163) 2026-01-05: [Helpers] *BREAKING CHANGE*: Vulkan:
//  Helper for creating render pass uses ImGui_ImplVulkanH_Window::AttachmentDesc to
//  create render pass. Removed ClearEnabled. (#9152) 2025-11-24: [Helpers] Vulkan: Helper
//  for creating a swap-chain (used by examples and multi-viewports) selects
//  VkSwapchainCreateInfoKHR's compositeAlpha based on cap.supportedCompositeAlpha.
//  (#8784) 2025-10-15: Vulkan: Added IMGUI_IMPL_VULKAN_VOLK_FILENAME to configure path to
//  volk.h header. (#9008) 2025-09-26: *BREAKING CHANGE*: moved some fields in
...
//  init_info.PipelineInfoMain.PipelineRenderingCreateInfo. 2025-09-26: *BREAKING CHANGE*:
//  renamed ImGui_ImplVulkan_MainPipelineCreateInfo to ImGui_ImplVulkan_PipelineInfo.
//  Introduced very recently so shouldn't affect many users. 2025-09-26: [Helpers]
//  *BREAKING CHANGE*: Helper ImGui_ImplVulkanH_CreateOrResizeWindow() added a
//  VkImageUsageFlags image_usage` argument, default to
//  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT if 0. 2025-09-26: Vulkan: Added a way to customize
//  shaders by filling
//  ImGui_ImplVulkan_InitInfo::CustomShaderVertCreateInfo/CustomShaderFragCreateInfo.
//  (#8585) 2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown. 2025-09-04:
//  Vulkan: Added ImGui_ImplVulkan_CreateMainPipeline(). (#8110, #8111) 2025-07-27:
//  Vulkan: Fixed texture update corruption introduced on 2025-06-11. (#8801, #8755,
//  #8840) 2025-07-07: Vulkan: Fixed texture synchronization issue introduced on
//  2025-06-11. (#8772) 2025-06-27: Vulkan: Fixed validation errors during texture
//  upload/update by aligning upload size to 'nonCoherentAtomSize'. (#8743, #8744)
//  2025-06-11: Vulkan: Added support for ImGuiBackendFlags_RendererHasTextures, for
//  dynamic font atlas. Removed ImGui_ImplVulkan_CreateFontsTexture() and
//  ImGui_ImplVulkan_DestroyFontsTexture(). 2025-05-07: Vulkan: Fixed validation errors
//  during window detach in multi-viewport mode. (#8600, #8176) 2025-05-07: Vulkan: Load
//  dynamic rendering functions using vkGetDeviceProcAddr() + try both non-KHR and KHR
//  versions. (#8600, #8326, #8365) 2025-04-07: Vulkan: Deep-copy
//  ImGui_ImplVulkan_InitInfo::PipelineRenderingCreateInfo's pColorAttachmentFormats
//  buffer when set, in order to reduce common user-error of specifying a pointer to data
//  that gets out of scope. (#8282) 2025-02-14: *BREAKING CHANGE*: Added uint32_t
//  api_version to ImGui_ImplVulkan_LoadFunctions(). 2025-02-13: Vulkan: Added ApiVersion
//  field in ImGui_ImplVulkan_InitInfo. Default to header version if unspecified. Dynamic
//  rendering path loads "vkCmdBeginRendering/vkCmdEndRendering" (without -KHR suffix) on
//  API 1.3. (#8326) 2025-01-09: Vulkan: Added
//  IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE to clarify how many image sampler
//  descriptors are expected to be available in descriptor pool. (#6642) 2025-01-06:
//  [Helpers] Vulkan: Added more ImGui_ImplVulkanH_XXXX helper functions to simplify our
//  examples. 2024-12-11: [Helpers] Vulkan: Fixed setting
//  VkSwapchainCreateInfoKHR::preTransform for platforms not supporting
//  VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR. (#8222) 2024-11-27: Vulkan: Make user-provided
//  descriptor pool optional. As a convenience, when setting init_info->DescriptorPoolSize
//  the backend will create one itself. (#8172, #4867) 2024-10-07: Vulkan: Changed default
//  texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: Vulkan: Expose selected
//  render state in ImGui_ImplVulkan_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07: Vulkan: Compiling
//  with '#define ImTextureID=ImU64' is unnecessary now that dear imgui defaults
//  ImTextureID to u64 instead of void*. 2024-04-19: Vulkan: Added convenience support for
//  Volk via IMGUI_IMPL_VULKAN_USE_VOLK define (you can also use
//  IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via ImGui_ImplVulkan_LoadFunctions().)
//  2024-02-14: *BREAKING CHANGE*: Moved RenderPass parameter from ImGui_ImplVulkan_Init()
//  function to ImGui_ImplVulkan_InitInfo structure. Not required when using dynamic
//  rendering. 2024-02-12: *BREAKING CHANGE*: Dynamic rendering now require filling
//  PipelineRenderingCreateInfo structure. 2024-01-19: [Helpers] Vulkan: Fixed
//  vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by allocating one extra
//  semaphore than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory() calls
//  unnecessarily using full buffer size (#3957). Fixed MinAllocationSize handing (#7189).
//  2024-01-03: Vulkan: Added MinAllocationSize field in ImGui_ImplVulkan_InitInfo to
//  workaround zealous "best practice" validation layer. (#7189, #4238) 2024-01-03:
//  Vulkan: Stopped creating command pools with
//  VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't reset them. 2023-11-29:
...
//  ImGui_ImplVulkan_CreateFontsTexture(): backend now creates its own command-buffer to
//  upload fonts.
//              *BREAKING CHANGE*: Removed ImGui_ImplVulkan_DestroyFontUploadObjects()
//              which is now unnecessary as we create and destroy those objects in the
//              backend. ImGui_ImplVulkan_CreateFontsTexture() is automatically called by
//              NewFrame() the first time. You can call
//              ImGui_ImplVulkan_CreateFontsTexture() again to recreate the font atlas
//              texture. Added ImGui_ImplVulkan_DestroyFontsTexture() but you probably
//              never need to call this.
//  2023-07-04: Vulkan: Added optional support for VK_KHR_dynamic_rendering. User needs to
//  set init_info->UseDynamicRendering = true and init_info->ColorAttachmentFormat.
//  2023-01-02: Vulkan: Fixed sampler passed to ImGui_ImplVulkan_AddTexture() not being
//  honored + removed a bunch of duplicate code. 2022-10-11: Using 'nullptr' instead of
//  'NULL' as per our switch to C++11. 2022-10-04: Vulkan: Added experimental
//  ImGui_ImplVulkan_RemoveTexture() for api symmetry. (#914, #5738). 2022-01-20: Vulkan:
//  Added support for ImTextureID as VkDescriptorSet. User need to call
//  ImGui_ImplVulkan_AddTexture(). Building for 32-bit targets requires '#define
//  ImTextureID ImU64'. (#914). 2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of
//  render a full-viewport to reduce likelihood of issues with people using
//  VK_DYNAMIC_STATE_SCISSOR in their app without calling vkCmdSetScissor() explicitly
//  every frame. 2021-06-29: Reorganized backend to pull data from a single structure to
//  facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX).
//  2021-03-22: Vulkan: Fix mapped memory validation error when buffer sizes are not
//  multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change
//  blending equation to preserve alpha in output buffer. 2021-01-27: Vulkan: Added
//  support for custom function load and IMGUI_IMPL_VULKAN_NO_PROTOTYPES by using
//  ImGui_ImplVulkan_LoadFunctions(). 2020-11-11: Vulkan: Added support for specifying
//  which subpass to reference during VkPipeline creation. 2020-09-07: Vulkan: Added
//  VkPipeline parameter to ImGui_ImplVulkan_RenderDrawData (default to one passed to
//  ImGui_ImplVulkan_Init). 2020-05-04: Vulkan: Fixed crash if initial frame has no
//  vertices. 2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be
//  called if the ImDrawData didn't have vertices. 2019-08-01: Vulkan: Added support for
//  specifying multisample count. Set ImGui_ImplVulkan_InitInfo::MSAASamples to one of the
//  VkSampleCountFlagBits values to use, default is non-multisampled as before.
//  2019-05-29: Vulkan: Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Vulkan: Added support for
//  special ImDrawCallback_ResetRenderState callback to reset render state. 2019-04-04:
//  *BREAKING CHANGE*: Vulkan: Added ImageCount/MinImageCount fields in
//  ImGui_ImplVulkan_InitInfo, required for initialization (was previously a hard #define
//  IMGUI_VK_QUEUED_FRAMES 2). Added ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04:
//  [Helpers] Vulkan: Added VkInstance argument to ImGui_ImplVulkanH_CreateWindow()
//  optional helper. 2019-04-04: Vulkan: Avoid passing negative coordinates to
//  vkCmdSetScissor, which debug validation layers do not like. 2019-04-01: Vulkan:
//  Support for 32-bit index buffer (#define ImDrawIdx unsigned int). 2019-02-16: Vulkan:
//  Viewport and clipping rectangles correctly using draw_data->FramebufferScale to allow
//  retina display. 2018-11-30: Misc: Setting up io.BackendRendererName so it can be
//  displayed in the About Window. 2018-08-25: [Helpers] Vulkan: Fixed mishandled
//  VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22: Inverted the parameters to
//  ImGui_ImplVulkan_RenderDrawData() to be consistent with other backends. 2018-06-08:
//  Misc: Extracted imgui_impl_vulkan.cpp/.h away from the old combined GLFW+Vulkan
//  example. 2018-06-08: Vulkan: Use draw_data->DisplayPos and draw_data->DisplaySize to
//  setup projection matrix and clipping rectangle. 2018-03-03: [Helpers] Vulkan: Various
//  refactor, created a couple of ImGui_ImplVulkanH_XXX helper that the example can use
//  and that viewport support will use. 2018-03-01: Vulkan: Renamed
//  ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and fields to match more
//  closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn
//  callback, ImGui_ImplVulkan_Render() calls ImGui_ImplVulkan_RenderDrawData() itself.
//  2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available from 1.60
//  WIP, user needs to call CreateContext/DestroyContext themselves. 2017-05-15: Vulkan:
//  Fix scissor offset being negative. Fix new Vulkan validation warnings. Set required
//  depth member for buffer image copy. 2016-11-13: Vulkan: Fix validation layer warnings
//  and errors and redeclare gl_PerVertex. 2016-10-18: Vulkan: Add location decorators &
//  change to use structs as in/out in glsl, update embedded spv (produced with
//  glslangValidator -x). Null the released resources. 2016-08-27: Vulkan: Fix Vulkan
//  example for use when a depth buffer is active.
...
#pragma clang diagnostic ignored \
    "-Wimplicit-int-float-conversion"  // warning: implicit conversion from 'xxx' to
                                       // 'float' may lose precision
#pragma clang diagnostic ignored \
    "-Wcast-function-type"  // warning: cast between incompatible function types (for
                            // loader)
#endif
...
struct ImGui_ImplVulkan_Data {
  ...
  ImGui_ImplVulkan_RenderState*
      RenderState;  // == ImGui::GetPlatformIO().Renderer_RenderState during rendering.
...
static uint32_t __glsl_shader_vert_spv[] = {
    0x07230203, 0x00010000, 0x0008000b, 0x0000002e, 0x00000000, 0x00020011, 0x00000001,
    ...
    0x00000001, 0x00030047, 0x00000019, 0x00000002, 0x00050048, 0x00000019, 0x00000000,
    0x0000000b, 0x00000000, 0x00040047, 0x0000001c, 0x0000001e, 0x00000000, 0x00030047,
    0x0000001e, 0x00000002, 0x00050048, 0x0000001e, 0x00000000, 0x00000023, 0x00000000,
    0x00050048, 0x0000001e, 0x00000001, 0x00000023, 0x00000008, 0x00020013, 0x00000002,
...
static uint32_t __glsl_shader_frag_spv[] = {
    0x07230203, 0x00010000, 0x0008000b, 0x00000023, 0x00000000, 0x00020011, 0x00000001,
    0x0006000b, 0x00000001, 0x4c534c47, 0x6474732e, 0x3035342e, 0x00000000, 0x0003000e,
    ...
    0x00030005, 0x0000000d, 0x00006e49, 0x00050005, 0x00000015, 0x7865545f, 0x65727574,
    0x00000000, 0x00050005, 0x00000019, 0x6d61535f, 0x72656c70, 0x00000000, 0x00040047,
    0x00000009, 0x0000001e, 0x00000000, 0x00040047, 0x0000000d, 0x0000001e, 0x00000000,
    0x00040047, 0x00000015, 0x00000021, 0x00000000, 0x00040047, 0x00000015, 0x00000022,
    0x00000000, 0x00040047, 0x00000019, 0x00000021, 0x00000000, 0x00040047, 0x00000019,
    0x00000022, 0x00000001, 0x00020013, 0x00000002, 0x00030021, 0x00000003, 0x00000002,
    ...
    0x00000003, 0x00040017, 0x0000000a, 0x00000006, 0x00000002, 0x0004001e, 0x0000000b,
    0x00000007, 0x0000000a, 0x00040020, 0x0000000c, 0x00000001, 0x0000000b, 0x0004003b,
    0x0000000c, 0x0000000d, 0x00000001, 0x00040015, 0x0000000e, 0x00000020, 0x00000001,
    0x0004002b, 0x0000000e, 0x0000000f, 0x00000000, 0x00040020, 0x00000010, 0x00000001,
    0x00000007, 0x00090019, 0x00000013, 0x00000006, 0x00000001, 0x00000000, 0x00000000,
    0x00000000, 0x00000001, 0x00000000, 0x00040020, 0x00000014, 0x00000000, 0x00000013,
    0x0004003b, 0x00000014, 0x00000015, 0x00000000, 0x0002001a, 0x00000017, 0x00040020,
    0x00000018, 0x00000000, 0x00000017, 0x0004003b, 0x00000018, 0x00000019, 0x00000000,
    0x0003001b, 0x0000001b, 0x00000013, 0x0004002b, 0x0000000e, 0x0000001d, 0x00000001,
    0x00040020, 0x0000001e, 0x00000001, 0x0000000a, 0x00050036, 0x00000002, 0x00000004,
    0x00000000, 0x00000003, 0x000200f8, 0x00000005, 0x00050041, 0x00000010, 0x00000011,
    0x0000000d, 0x0000000f, 0x0004003d, 0x00000007, 0x00000012, 0x00000011, 0x0004003d,
    0x00000013, 0x00000016, 0x00000015, 0x0004003d, 0x00000017, 0x0000001a, 0x00000019,
    0x00050056, 0x0000001b, 0x0000001c, 0x00000016, 0x0000001a, 0x00050041, 0x0000001e,
    0x0000001f, 0x0000000d, 0x0000001d, 0x0004003d, 0x0000000a, 0x00000020, 0x0000001f,
    0x00050057, 0x00000007, 0x00000021, 0x0000001c, 0x00000020, 0x00050085, 0x00000007,
    0x00000022, 0x00000012, 0x00000021, 0x0003003e, 0x00000009, 0x00000022, 0x000100fd,
    0x00010038};
...
// Backend data stored in io.BackendRendererUserData to allow support for multiple Dear
// ImGui contexts It is STRONGLY preferred that you use docking branch with
// multi-viewports (== single Dear ImGui context + multiple windows) instead of multiple
// Dear ImGui contexts.
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline,
  ...
  vkCmdPushConstants(command_buffer, bd->PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0,
                     sizeof(float) * 4, constants);

  // Setup sampler
  vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          bd->PipelineLayout, 1, 1, &bd->SamplerLinearDS, 0, nullptr);
}

// Draw callbacks
static void ImGui_ImplVulkan_DrawCallback_ResetRenderState(const ImDrawList*,
                                                           const ImDrawCmd*) {
}  // Intentionally empty. Used as an identifier for rendering loop to call its code.
   // Simpler to implement this way.
static void ImGui_ImplVulkan_DrawCallback_SetSamplerLinear(const ImDrawList*,
                                                           const ImDrawCmd*) {
  ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
  vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          bd->PipelineLayout, 1, 1, &bd->SamplerLinearDS, 0, nullptr);
}
static void ImGui_ImplVulkan_DrawCallback_SetSamplerNearest(const ImDrawList*,
                                                            const ImDrawCmd*) {
  ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
  vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          bd->PipelineLayout, 1, 1, &bd->SamplerNearestDS, 0, nullptr);
}
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data,
  ...
  if (draw_data->TotalVtxCount > 0) {
    ...
    VkDeviceSize vertex_size = AlignBufferSize(
        draw_data->TotalVtxCount * sizeof(ImDrawVert), bd->BufferMemoryAlignment);
    VkDeviceSize index_size = AlignBufferSize(
        draw_data->TotalIdxCount * sizeof(ImDrawIdx), bd->BufferMemoryAlignment);
    ...
    VkResult err = vkMapMemory(v->Device, rb->VertexBufferMemory, 0, vertex_size, 0,
                               (void**)&vtx_dst);
    check_vk_result(err);
    err =
        vkMapMemory(v->Device, rb->IndexBufferMemory, 0, index_size, 0, (void**)&idx_dst);
    check_vk_result(err);
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        if (image_view != last_image_view)
          vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                  bd->PipelineLayout, 0, 1, &image_view, 0, nullptr);
  ...
  // - Your app uses a pipeline with VK_DYNAMIC_STATE_VIEWPORT or VK_DYNAMIC_STATE_SCISSOR
  // dynamic state
  // - And you forgot to call vkCmdSetViewport() and vkCmdSetScissor() yourself to
  // explicitly set that state. If you use VK_DYNAMIC_STATE_VIEWPORT or
  // VK_DYNAMIC_STATE_SCISSOR you are responsible for setting the values before rendering.
  // In theory we should aim to backup/restore those values but I am not sure this is
  // possible. We perform a call to vkCmdSetScissor() to set back a full viewport which is
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    backend_tex->DescriptorSet = ImGui_ImplVulkan_AddTexture(
        backend_tex->ImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  ...
  if (tex->Status == ImTextureStatus_WantCreate ||
    ...
    // Update full texture or selected blocks. We only ever write to textures regions
    // which have never been used before! This backend choose to use tex->UpdateRect but
    // you can use tex->Updates[] to upload individual regions. We could use the smaller
    // rect on _WantCreate but using the full rect allows us to clear the texture.
    const int upload_x =
...
static void ImGui_ImplVulkan_CreateShaderModules(VkDevice device,
  ...
  if (bd->ShaderModuleVert == VK_NULL_HANDLE) {
    ...
    VkShaderModuleCreateInfo* p_vert_info = (v->CustomShaderVertCreateInfo.sType ==
                                             VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
                                                ? &v->CustomShaderVertCreateInfo
                                                : &default_vert_info;
    VkResult err =
        vkCreateShaderModule(device, p_vert_info, allocator, &bd->ShaderModuleVert);
    check_vk_result(err);
  }
  if (bd->ShaderModuleFrag == VK_NULL_HANDLE) {
    ...
    VkShaderModuleCreateInfo* p_frag_info = (v->CustomShaderFragCreateInfo.sType ==
                                             VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO)
                                                ? &v->CustomShaderFragCreateInfo
                                                : &default_frag_info;
    VkResult err =
        vkCreateShaderModule(device, p_frag_info, allocator, &bd->ShaderModuleFrag);
    check_vk_result(err);
  }
}
...
#endif
  VkPipeline pipeline;
  VkResult err = vkCreateGraphicsPipelines(device, pipelineCache, 1, &create_info,
                                           allocator, &pipeline);
  check_vk_result(err);
  return pipeline;
}

static void ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDescriptorSetLayout* p_layout,
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
  ...
  if (!bd->DescriptorSetLayoutTexture)
    ImGui_ImplVulkan_CreateDescriptorSetLayout(&bd->DescriptorSetLayoutTexture,
                                               VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
  if (!bd->DescriptorSetLayoutSampler)
    ImGui_ImplVulkan_CreateDescriptorSetLayout(&bd->DescriptorSetLayoutSampler,
  ...
  if (v->DescriptorPoolSize != 0) {
    ...
    pool_info.maxSets =
        v->DescriptorPoolSize + IMGUI_IMPL_VULKAN_MINIMUM_SAMPLER_POOL_SIZE;
    pool_info.poolSizeCount = (uint32_t)IM_COUNTOF(pool_sizes);
    pool_info.pPoolSizes = pool_sizes;
    err =
        vkCreateDescriptorPool(v->Device, &pool_info, v->Allocator, &bd->DescriptorPool);
    check_vk_result(err);
  }
  ...
  if (!bd->PipelineLayout) {
    // Constants: we are using 'vec2 offset' and 'vec2 scale' instead of a full 3d
    // projection matrix
    VkPushConstantRange push_constants[1] = {};
    push_constants[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    push_constants[0].offset = sizeof(float) * 0;
    push_constants[0].size = sizeof(float) * 4;
    VkDescriptorSetLayout set_layout[2] = {bd->DescriptorSetLayoutTexture,
                                           bd->DescriptorSetLayoutSampler};
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
  create_main_pipeline |= (v->UseDynamicRendering &&
...
void ImGui_ImplVulkan_DestroyDeviceObjects() {
  ...
  if (bd->SamplerLinear) {
    vkDestroySampler(v->Device, bd->SamplerLinear, v->Allocator);
    bd->SamplerLinear = VK_NULL_HANDLE;
  }
  if (bd->SamplerNearest) {
    vkDestroySampler(v->Device, bd->SamplerNearest, v->Allocator);
    bd->SamplerNearest = VK_NULL_HANDLE;
  }
  ...
  if (bd->DescriptorSetLayoutTexture) {
    vkDestroyDescriptorSetLayout(v->Device, bd->DescriptorSetLayoutTexture, v->Allocator);
    bd->DescriptorSetLayoutTexture = VK_NULL_HANDLE;
  }
  if (bd->DescriptorSetLayoutSampler) {
    vkDestroyDescriptorSetLayout(v->Device, bd->DescriptorSetLayoutSampler, v->Allocator);
    bd->DescriptorSetLayoutSampler = VK_NULL_HANDLE;
  }
  if (bd->PipelineLayout) {
...
static void ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
  ...
  // - Try loading core (non-KHR) versions first (this will work for Vulkan 1.3+ and the
  // device supports dynamic rendering)
  ...
  // - Fallback to KHR versions if core not available (this will work if KHR extension is
  // available and enabled and also the device supports dynamic rendering)
...
// We don't care about other versions than 1.3 for our checks, so don't need to make this
// exhaustive (e.g. with all #ifdef VK_VERSION_1_X checks)
...
bool ImGui_ImplVulkan_LoadFunctions(
  ...
  //      ImGui_ImplVulkan_LoadFunctions(VK_API_VERSION_1_3, [](const char* function_name,
  //      void*) { return vkGetInstanceProcAddr(your_vk_isntance, function_name); });
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info) {
  ...
  platform_io.DrawCallback_ResetRenderState =
      ImGui_ImplVulkan_DrawCallback_ResetRenderState;
  platform_io.DrawCallback_SetSamplerLinear =
      ImGui_ImplVulkan_DrawCallback_SetSamplerLinear;
  platform_io.DrawCallback_SetSamplerNearest =
      ImGui_ImplVulkan_DrawCallback_SetSamplerNearest;
  ...
  if (info->DescriptorPool !=
      VK_NULL_HANDLE)  // Either DescriptorPool or DescriptorPoolSize must be set, not
                       // both!
  ...
  if (!ImGui_ImplVulkan_CreateDeviceObjects())
    IM_ASSERT(0 &&
              "ImGui_ImplVulkan_CreateDeviceObjects() failed!");  // <- Can't be hit yet.
...
// FIXME: This is experimental in the sense that we are unsure how to best design/tackle
// this problem, please post to https://github.com/ocornut/imgui/pull/914 if you have
// suggestions.
VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkImageView image_view,
...
void ImGui_ImplVulkan_DestroyWindowRenderBuffers(
  ...
  for (uint32_t n = 0; n < buffers->Count; n++)
    ImGui_ImplVulkan_DestroyFrameRenderBuffers(device, &buffers->FrameRenderBuffers[n],
                                               allocator);
...
// (Used by example's main.cpp. Used by multi-viewport features. PROBABLY NOT used by your
// own engine/app.)
//-------------------------------------------------------------------------
// You probably do NOT need to use or care about those functions.
// Those functions only exist because:
//   1) they facilitate the readability and maintenance of the multiple main.cpp examples
//   files. 2) the upcoming multi-viewport feature will need them internally.
// Generally we avoid exposing any kind of superfluous high-level helpers in the backends,
// but it is too much code to duplicate everywhere so we exceptionally expose them.
//
// Your engine/app will likely _already_ have code to setup all that stuff (swap chain,
// render pass, frame buffers, etc.). You may read this code to learn about Vulkan, but it
// is recommended you use your own custom tailored code to do equivalent work. (The
// ImGui_ImplVulkanH_XXX functions do not interact with any of the state used by the
// regular ImGui_ImplVulkan_XXX functions)
//-------------------------------------------------------------------------
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(
  ...
  // VK_IMAGE_CREATE_MUTABLE_BIT was set at image creation Assuming that the default
  // behavior is without setting this bit, there is no need for separate Swapchain image
  // and image view format Additionally several new color spaces were introduced with
  // Vulkan Spec v1.0.40, hence we must make sure that a format with the mostly available
  // color space, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR, is found and used.
  ...
  // First check if only one format, VK_FORMAT_UNDEFINED, is available, which would imply
...
VkPhysicalDevice ImGui_ImplVulkanH_SelectPhysicalDevice(VkInstance instance) {
  ...
  // available. This covers most common cases (multi-gpu/integrated+dedicated graphics).
  // Handling more complicated setups (multiple dedicated GPUs) is out of scope of this
  // sample.
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device,
  ...
  // We don't use ImGui_ImplVulkanH_DestroyWindow() because we want to preserve the old
  // swapchain to create the new one. Destroy old Framebuffer
  for (uint32_t i = 0; i < wd->ImageCount; i++)
  ...
  // If min image count was not specified, request different count of images dependent on
...
// - 2025/09/26: v1.92.4 added a trailing 'VkImageUsageFlags image_usage' parameter which
// is usually VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT.
void ImGui_ImplVulkanH_CreateOrResizeWindow(
  ...
  ImGui_ImplVulkanH_CreateWindowSwapChain(physical_device, device, wd, allocator, width,
  ...
  // ImGui_ImplVulkanH_CreateOrResizeWindow function is called before the
  // ImGui_ImplVulkan_Init function, so we don't have access to the queue yet. Here we
  // have the queue_family that we can use to grab a queue from the device and submit the
  // command buffer. It would be better to have access to the queue as suggested in the
  // FIXME below.
...
void ImGui_ImplVulkanH_DestroyWindow(VkInstance instance, VkDevice device,
                                     ...
                                     const VkAllocationCallbacks* allocator) {
  IM_UNUSED(instance);
  vkDeviceWaitIdle(device);  // FIXME: We could wait on the Queue if we had the queue in
  ...
  // vkDestroySurfaceKHR(instance, wd->Surface, allocator); // v1.92.6 (~2026-01-16):
  // because wd->Surface is user provided we don't attempt to destroy it ourself.
}
